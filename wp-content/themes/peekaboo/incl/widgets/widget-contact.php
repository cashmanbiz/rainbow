...
class pkb_widget_contact extends WP_Widget
{
    ...
    function pkb_widget_contact()
    {
        ...
        $this->WP_Widget('pkb_widget_contact', __('Peekaboo Contact', 'peekaboo'), $widget_ops);
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id('code'); ?>"><?php _e(
                    'Google Map embed code (with iframe):',
                    'peekaboo'
                ) ?></label>
    ...
    function widget($args, $instance)
    {
        ...
        if ($code) {
            echo do_shortcode(
                '[reveal link="' . $map_title
                    . '" linkclass="modalmap" class="medium"]<img src="http://placehold.it/800x600"/>' . $code
                    . '[/reveal]'
            );
        }
        ?>
