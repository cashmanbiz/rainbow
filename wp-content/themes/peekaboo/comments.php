...
<li <?php comment_class(); ?>>
  ...
  <!-- JJC removing closed Comments 02-Aug-15		<section id="comments">
                <div class="notice">
                    <p class="bottom"><?php //  _e('Comments are closed.', 'peekaboo') 					?></p>
                </div>
            </section>	-->
        <?php endif; ?>
    <?php endif; ?>
    <?php if (comments_open()) : ?>
        <section id="respond">
            <h3 class="replace"><?php comment_form_title(__('Leave a Reply', 'peekaboo'), __('Leave a Reply to %s', 'peekaboo')); ?></h3>

            <p class="cancel-comment-reply"><?php cancel_comment_reply_link(); ?></p>
            <?php if (get_option('comment_registration') && !is_user_logged_in()) : ?>
                <p><?php printf(__('You must be <a href="%s">logged in</a> to post a comment.', 'peekaboo'), wp_login_url(get_permalink())); ?></p>
            <?php else : ?>
                <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
                    <?php if (is_user_logged_in()) : ?>
                        <p><?php printf(__('Logged in as <a href="%s/wp-admin/profile.php">%s</a>.', 'peekaboo'), get_option('siteurl'), $user_identity); ?>
                            <a href="<?php echo wp_logout_url(get_permalink()); ?>"
                               title="<?php __('Log out of this account', 'peekaboo'); ?>"><?php _e('Log out &raquo;', 'peekaboo'); ?></a>
                        </p>
                    <?php else : ?>
                        <p>
                            <label
                                for="author"><?php _e('Name', 'peekaboo'); if ($req) _e(' (required)', 'peekaboo'); ?></label>
                        ...
                        <p>
                            <label
                                for="email"><?php _e('Email (will not be published)', 'peekaboo'); if ($req) _e(' (required)', 'peekaboo'); ?></label>
                        ...
                        <p>
                            <label for="url"><?php _e('Website', 'peekaboo'); ?></label>
                            <input type="text" class="five" name="url" id="url"
                                   value="<?php echo esc_attr($comment_author_url); ?>" size="22" tabindex="3">
                        </p>
                    <?php endif; ?>
                    <p>
                        <label for="comment"><?php _e('Comment', 'peekaboo'); ?></label>
                        <textarea name="comment" id="comment" tabindex="4" rows="4"></textarea>
                    </p>

                    <p id="allowed_tags" class="small"><strong>XHTML:</strong> You can use these tags:
                        <code><?php echo allowed_tags(); ?></code></p>

                    <p><input name="submit" class="button replace" type="submit" id="submit" tabindex="5"
                              value="<?php _e('Submit Comment', 'peekaboo'); ?>"></p>
